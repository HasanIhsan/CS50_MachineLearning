...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        # Get the maximum Q-value for all available actions
    ...
    def choose_action(self, state, epsilon=True):
        ...
        # epsilon-greedy: radom choice with probability epsilon
        if epsilon and random.random() < self.epsilon:
            return random.choice(actions)
        
        #otherwiser choose best action greedily
        q_values = [(self.get_q_value(state, action), action) for action in actions]
        
        #find max Q-value
        max_q = max(q_values, key=lambda x: x[0])[0]
        
        #filter actions that have the max Q-value
